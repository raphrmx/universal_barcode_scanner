...
public class SwiftUniversalBarcodeScannerPlugin: NSObject, FlutterPlugin, ScanBarcodeDelegate,FlutterStreamHandler {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let channel = FlutterMethodChannel(name: "universal_barcode_scanner", binaryMessenger: registrar.messenger())
        let instance = SwiftUniversalBarcodeScannerPlugin()
        registrar.addMethodCallDelegate(instance, channel: channel)
        let eventChannel=FlutterEventChannel(name: "universal_barcode_scanner/events", binaryMessenger: registrar.messenger())
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        SwiftUniversalBarcodeScannerPlugin.barcodeStream = events
        return nil
    }
    
    public func onCancel(withArguments arguments: Any?) -> FlutterError? {
        SwiftUniversalBarcodeScannerPlugin.barcodeStream=nil
        return nil
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let colorCode = args["lineColor"] as? String{
            SwiftUniversalBarcodeScannerPlugin.lineColor = colorCode
        }else {
            SwiftUniversalBarcodeScannerPlugin.lineColor = "#ff6666"
        }
        if let buttonText = args["cancelButtonText"] as? String{
            SwiftUniversalBarcodeScannerPlugin.cancelButtonText = buttonText
        }else {
            SwiftUniversalBarcodeScannerPlugin.cancelButtonText = "Cancel"
        }
        if let flashStatus = args["isShowFlashIcon"] as? Bool{
            SwiftUniversalBarcodeScannerPlugin.isShowFlashIcon = flashStatus
        }else {
            SwiftUniversalBarcodeScannerPlugin.isShowFlashIcon = false
        }
        if let isContinuousScan = args["isContinuousScan"] as? Bool{
            SwiftUniversalBarcodeScannerPlugin.isContinuousScan = isContinuousScan
        }else {
            SwiftUniversalBarcodeScannerPlugin.isContinuousScan = false
        }

         if let delayMillis = args["delayMillis"] as? Int{
                    SwiftUniversalBarcodeScannerPlugin.delayMillis = delayMillis
                }else {
                    SwiftUniversalBarcodeScannerPlugin.delayMillis = 0
                }

        
        if let scanModeReceived = args["scanMode"] as? Int {
            if scanModeReceived == ScanMode.DEFAULT.index {
                SwiftUniversalBarcodeScannerPlugin.scanMode = ScanMode.QR.index
            }else{
                SwiftUniversalBarcodeScannerPlugin.scanMode = scanModeReceived
            }
        }else{
            SwiftUniversalBarcodeScannerPlugin.scanMode = ScanMode.QR.index
        }
        ...
        if checkCameraAvailability(){
            if checkForCameraPermission() {
                SwiftUniversalBarcodeScannerPlugin.viewController.present(controller
                                                                        , animated: true) {
                ...
                AVCaptureDevice.requestAccess(for: .video) { success in
                    DispatchQueue.main.async {
                        if success {
                            ...
                            SwiftUniversalBarcodeScannerPlugin.viewController.present(alert, animated: true)
                        }
                    }
                }}
        }else {
    ...
    func showAlertDialog(title:String,message:String){
        ...
        SwiftUniversalBarcodeScannerPlugin.viewController.present(alertController, animated: true, completion: nil)
    }
}
...
class BarcodeScannerViewController: UIViewController {
    ...
    public lazy var cancelButton: UIButton! = {
        let view = UIButton()
        view.setTitle(SwiftUniversalBarcodeScannerPlugin.cancelButtonText, for: .normal)
        view.translatesAutoresizingMaskIntoConstraints = false
    ...
    func initUIComponents(){
        if isOrientationPortrait {
            screenHeight = (CGFloat)((SwiftUniversalBarcodeScannerPlugin.scanMode == ScanMode.QR.index) ? (screenSize.width * 0.8) : (screenSize.width * 0.5))
            
        } else {
            screenHeight = (CGFloat)((SwiftUniversalBarcodeScannerPlugin.scanMode == ScanMode.QR.index) ? (screenSize.height * 0.6) : (screenSize.height * 0.5))
        }
    ...
    func drawUIOverlays(withCompletion processCompletionCallback: () -> Void){
        ...
        if let qrCodeFrameView = qrCodeFrameView {
            ...
            if(!SwiftUniversalBarcodeScannerPlugin.isShowFlashIcon){
    ...
    @IBAction private func cancelButtonClicked() {
        if SwiftUniversalBarcodeScannerPlugin.isContinuousScan{
            self.dismiss(animated: true, completion: {
                SwiftUniversalBarcodeScannerPlugin.onBarcodeScanReceiver(barcode: "-2")
            })
        }else{
    ...
    private func drawLine() {
        ...
        scanLine.backgroundColor = hexStringToUIColor(hex: SwiftUniversalBarcodeScannerPlugin.lineColor)
        ...
        if SwiftUniversalBarcodeScannerPlugin.scanMode == ScanMode.QR.index {
...
extension BarcodeScannerViewController: AVCaptureMetadataOutputObjectsDelegate {
    public func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if metadataObj.stringValue != nil {
            ...
            SwiftUniversalBarcodeScannerPlugin.delayTimer?.invalidate()

            // Start a new timer
            SwiftUniversalBarcodeScannerPlugin.delayTimer = Timer.scheduledTimer(withTimeInterval: Double(SwiftUniversalBarcodeScannerPlugin.delayMillis/1000), repeats: false) { [weak self] _ in
     ...
     private func processBarcode(_ barcode: String) {
            if SwiftUniversalBarcodeScannerPlugin.isContinuousScan {
                SwiftUniversalBarcodeScannerPlugin.onBarcodeScanReceiver(barcode: barcode)
            } else {
...
extension BarcodeScannerViewController{
    ...
    func updateUIAfterRotation(){
        DispatchQueue.main.async {
            ...
            if UIDevice.current.orientation == .portrait || UIDevice.current.orientation == .portraitUpsideDown {
                self.screenHeight = (CGFloat)((SwiftUniversalBarcodeScannerPlugin.scanMode == ScanMode.QR.index) ? (self.screenSize.width * 0.8) : (self.screenSize.width * 0.5))
                
            } else {
                self.screenHeight = (CGFloat)((SwiftUniversalBarcodeScannerPlugin.scanMode == ScanMode.QR.index) ? (self.screenSize.height * 0.6) : (self.screenSize.height * 0.5))
            }
